...
The destructive shortcuts above are also enforced mechanically: the plugin's PreToolUse hook (`hooks/guard.sh`) denies force-push, `reset --hard`, `clean -f`, `restore` and `checkout -- <path>` (discarding uncommitted changes), `--no-verify` (and the `-c core.hooksPath=` bypass), `stash drop`/`clear`, and `branch -D` for the lead and every role session. Do not look for a way around a denial; report it and let the user run the command themselves (`! <command>` in the prompt) if they truly want it.
