...
- `done`: accepted scope is implemented, fresh required verification passed, both review gates passed on a delivered reviewer memo, final review has no unresolved material finding, memory is updated, all tasks are settled, and every role session is confirmed stopped through graceful shutdown or a confirmed force-stop.
- `done-with-risks`: implementation is complete but a named verification or external-state check could not run, a role session stop could not be confirmed after a bounded shutdown timeout, or a required role session (reviewer included) was force-closed or never delivered its memo — a missing reviewer memo can never yield `done`; record the evidence gap or active-session risk.
...
The destructive shortcuts above are also enforced mechanically: the plugin's PreToolUse hook (`hooks/guard.sh`) denies force-push, `reset --hard`, `clean -f`, the `restore`, `checkout`, and `switch` forms that discard uncommitted changes, `--no-verify` (and the `-c core.hooksPath=` bypass), `stash drop`/`clear`, and `branch -D` for the lead and every role session. Do not look for a way around a denial. When you decline or a hook denies a command, tell the user the exact command to run themselves prefixed with `! ` (for example `! git reset --hard`).
...
- Every spawn must select the role definition: pass `agent_type: "team-pl-<role>"` to `spawn_agent` (the host resolves the custom agent by its `name` field and applies the TOML's `developer_instructions` and, where set, `model_reasoning_effort`; the model is inherited from your session). Do not restate the role's lane in your own words and do not pass `model` or `reasoning_effort` overrides — the definition is the lane. A session spawned without its definition is a labeled lead role pass, not a role session, and the feature note must say so. The TOML's `sandbox_mode` is NOT applied when the parent session runs with an explicit sandbox override; if the parent is not read-only, record `role sandbox: parent override <mode>; TOML read-only not applied` under Team Lifecycle in the feature note. Fill the note's `Role sandbox:` line with the actual parent sandbox mode before closing.
- When the parent session is not read-only, reviewer/analyst roles have no sandbox guarantee — say so in the feature note's Team Lifecycle before spawning, and forbid file writes explicitly in every review/analysis role's brief.
